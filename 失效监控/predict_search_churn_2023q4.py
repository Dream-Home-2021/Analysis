...
"""
================================================================================
大搜客户 2023 年 Q4 季度失效预警逻辑回归预测系统
================================================================================
数据输入：
  - 2022 年 Q3 依赖：202207.csv, 202208.csv, 202209.csv (用于构建同比基准与老客特征)
  - 2023 年 Q3 依赖：202307.csv, 202308.csv, 202309.csv (用于提取行为、动量、断流特征)
  - 2023 年 Q4 实际：202310.csv, 202311.csv, 202312.csv (用于模型跨期样本外 OOT 回测验证)

核心流程：
  1. 数据清洗与多维流水解析 (按账户 ID 聚合并提取 RFM 活跃度、断流天数、衰减率、优惠券依赖度)
  2. 融入跨年同比与生命周期老客标识
  3. 逻辑回归 (Logistic Regression) 拟合与特征优势比 (Odds Ratio) 风险归因
  4. 生成 2023Q4 大搜客户失效概率预警名单、风险分级 (高危/中危/低危) 与单客诊断
  5. 自动执行 2023Q4 真实数据回测与分类效果评估 (ROC-AUC / Precision / Recall / F1)
================================================================================
"""
